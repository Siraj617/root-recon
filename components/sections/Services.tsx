...
const services = [
  {
    ...
    image:
      "https://www.shutterstock.com/image-vector/cyber-security-levels-knob-button-600nw-2508942001.jpg",
  },
  {
    title: "API Security",
    icon: Server,
    ...
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRXh6f5EQNCvTpknRA8YAt6CE0LjQbx1tThsg&s",
  },
  {
    ...
    image:
      "https://valencynetworks.com/wp-content/uploads/2025/10/mobile-app-vat-companies.jpg",
  },
  {
...
export function Services() {
  return (
    <section data-theme="dark" id="services" className="py-24 bg-black">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            <Link
              href={service.href}
              key={index}
              className="bg-white rounded-2xl p-5 border border-zinc-800 shadow-sm hover:shadow-lg transition-all group flex flex-col h-full"
            >
              {/* Image */}
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 mb-4">
                {service.features.map((feature, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    <CheckCircle2
                      size={12}
                      className="text-red-500/80 flex-shrink-0"
                    />
                    <span className="text-xs text-slate">{feature}</span>
                  </div>
                ))}
              </div>
            </Link>
